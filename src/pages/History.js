...
const History = () => {
  ...
  const deleteHistory = () => {
    const url = `${process.env.REACT_APP_BACKEND_HOST}/transaction/${transactionId}`;
    const getToken = localStorage.getItem("token");
    Axios.delete(url, {
      headers: { "x-access-token": getToken },
    })
  ...
  useEffect(() => {
    getHistory(
      `${process.env.REACT_APP_BACKEND_HOST}/transaction${
        localStorage.getItem("role") === "admin"
          ? "/all"
          : "/history?page=1&limit=9"
      }`
    );
    console.log(history);
  }, [deps]);
  ...
  titlebar("Coffeeshop | History");
  return (
    <>
      {localStorage.getItem("role") === "admin" ? <NavbarAdmin /> : <Navbar />}
      ...
      >
        ...
        <Modal.Body>
          do you want to{" "}
          {localStorage.getItem("role") === "admin" ? "changed" : "delete"}{" "}
          history data?
        </Modal.Body>
        <Modal.Footer>
          <Button
