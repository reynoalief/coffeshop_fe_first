...
const Product = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_BACKEND_HOST}/promos/all`)
      .then((res) => {
  ...
  const getCategory = () => {
    ...
    setCurrentPage(1);
    axios
      .get(`${url}?category=${category}&sorting=${sorting}&page=${1}&limit=8`)
      .then((res) => {
        ...
        //console.log(res.data.result);
        setLoading(false);
        navigate(`?category=${category}&sorting=${sorting}&page=${1}&limit=8`);
      })
  ...
  const getPrevProducts = () => {
    ...
    navigate(
      `?category=${category}&sorting=${sorting}&page=${currentPage - 1}&limit=8`
    );
    axios
      .get(prev)
      .then((res) => {
